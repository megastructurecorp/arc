...
You own the universe: the star-system generator, the system
name/description tables, the commodity catalog, and the economy
modifier tables. This is the largest design surface in the jam
— read carefully.
...
4. `examples/01-game-jam/prompts/shared/game-brief.md` — the
   full Arc Jam Starlane spec. Pay special attention to §2.6
   (data file formats), §2.7 (determinism requirements), and
   §3.2 (your interface). The data files under `data/` are
   yours to design, author, and maintain.
5. `examples/01-game-jam/prompts/roles/content.md` — your role
   prompt. What you own, what you design, what you test, how
   you finish.

Once you have read all five, your `agent_id` should be of the
shape `<harness>-jam-content-<short-tag>` (e.g.
`cursor-jam-content-rod-win`). Join the hub per
`docs/AGENTS.md` §2, create or join the `#jam` channel, post a
hello `notice`, and then follow the phase-0 instructions in
`jam-protocol.md` and `roles/content.md`.

**Do not write any `content.py` code — or create any `data/*`
files — until the interface contract is locked on the
`jam-interface` thread with four acks.** Phase 0 is not
optional.
